...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        packed_attn_output_ = packed_attn_output.new_zeros(packed_attn_output.shape[0], self.hidden_size)
        if mode == 'token_ar':
            packed_attn_output_ = self.o_proj(packed_attn_output)
        elif mode == 'block_ar':
            packed_attn_output_[packed_text_indexes] = self.o_proj(packed_attn_output[packed_text_indexes])
            packed_attn_output_[packed_vae_token_indexes] = self.o_proj_block_ar(packed_attn_output[packed_vae_token_indexes])
...
class Qwen2MoTDecoderLayer(nn.Module):
    ...
    def forward_train(
        ...
        packed_sequence_[packed_block_ar_indexes] = self.input_layernorm_block_ar(packed_sequence[packed_block_ar_indexes],t=packed_all_timestep_embeds[packed_block_ar_indexes] if packed_all_timestep_embeds is not None else None)
        ...
        packed_sequence_[packed_block_ar_indexes] = self.mlp_block_ar(
            self.post_attention_layernorm_block_ar(packed_sequence[packed_block_ar_indexes],t=packed_all_timestep_embeds[packed_block_ar_indexes] if packed_all_timestep_embeds is not None else None)
        )
        packed_sequence = residual + packed_sequence_
    ...
    def forward_inference(
        ...
        if mode == "token_ar":
            ...
            packed_query_sequence_[packed_vae_token_indexes] = self.input_layernorm_block_ar(packed_query_sequence[packed_vae_token_indexes], t=packed_all_timestep_embeds[packed_vae_token_indexes] if packed_all_timestep_embeds is not None else None)
        ...
        if mode == "token_ar":
            ...
            packed_vae_query_sequence = self.post_attention_layernorm_block_ar(packed_vae_query_sequence, t=packed_all_timestep_embeds[packed_vae_token_indexes] if packed_all_timestep_embeds is not None else None).to(torch.bfloat16)
...
class Qwen2MoEDecoderLayer(nn.Module):
    ...
    def forward_train(
        ...
        packed_sequence_token_ar = self.mlp(packed_sequence[packed_token_ar_indexes])
        packed_sequence_block_ar = self.mlp_block_ar(packed_sequence[packed_block_ar_indexes])
        packed_sequence_new[packed_token_ar_indexes] = packed_sequence_token_ar
        packed_sequence_new[packed_block_ar_indexes] = packed_sequence_block_ar
    ...
    def forward_inference(
        ...
        if mode == "token_ar":
            ...
            packed_query_sequence_[packed_vae_token_indexes] = self.mlp_block_ar(packed_query_sequence[packed_vae_token_indexes])
            packed_query_sequence = packed_query_sequence_
        packed_query_sequence = residual + packed_query_sequence
