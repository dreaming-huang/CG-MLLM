...
class Qwen2RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states,t=None):
...
class Qwen2AdaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        super().__init__()
        self.weight = nn.Parameter(torch.ones(hidden_size))
        self.variance_epsilon = eps
    ...
    def forward(self, hidden_states,t):
        ...
        #ada scale and shift
        emb = self.linear(self.silu(t).to(hidden_states.dtype))
        scale, shift = torch.chunk(emb, 2, dim=1)
        # implement of : self.norm(x) * (1 + scale)[:, None, :] + shift[:, None, :],hidden_states already norm, re scale and shift
...
class Qwen2VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2VLConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=getattr(config,"attention_bias",True))
        self.k_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=getattr(config,"attention_bias",True))
        self.v_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=getattr(config,"attention_bias",True))
    ...
    def forward(
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]  # mask down-weights unseen prefix tokens
...
class Qwen2VLSdpaAttention(Qwen2VLAttention):
    ...
    def forward(
        ...
        is_causal = True if causal_mask is None and q_len > 1 else False  # causal mask has a large effect
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if attention_mask is not None:
                attention_mask = attention_mask.to(inputs_embeds.device)  # attention_mask marks padding tokens
