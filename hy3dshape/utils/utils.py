...
def smart_load_model(
    ...
    if not os.path.exists(model_path):
        ...
        try:
            ...
            # Download only the requested subfolder.
            path = snapshot_download(
                repo_id=original_model_path,
                allow_patterns=[f"{subfolder}/*"],  # match files inside the subfolder
                local_dir=model_fld 
            )
            model_path = os.path.join(path, subfolder)  # keep the joined path unchanged
        except ImportError:
            logger.warning(
