...
def extract_near_surface_volume_fn(input_tensor: torch.Tensor, alpha: float):
    ...
    valid_mask = val > -9000  # treat -9000 as invalid

    # Neighbor lookup that keeps tensor ranks aligned.
    def get_neighbor(t, shift, axis):
        """Shift along one axis while keeping the tensor rank unchanged."""
        if shift == 0:
            return t.clone()

        # Pad axes for an input of shape [D, D, D] mapped to z, y, x.
        pad_dims = [0, 0, 0, 0, 0, 0]  # format: [x_before, x_after, y_before, y_after, z_before, z_after]

        # Choose padding from the axis.
        ...
        # Apply the slice and restore the original rank.
        padded = padded.squeeze(0).squeeze(0)
        sliced = padded[slice_dims]
        return sliced

    # Collect neighbors on each axis.
    ...
    # Replace invalid boundary values while keeping ranks aligned.
    ...
    # Measure sign consistency in float32.
